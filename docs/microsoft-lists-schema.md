...
This is the target central-storage structure for the audit assignment tracker. The current app still runs from browser storage, but Admin and Audit Manager users can export a Microsoft Lists package containing the list schemas, Graph-style list creation payloads, and seed rows.
...
| Audit Team Members | Lead/support workload rows | `TrackerAssignmentId`, `PersonName`, `TeamRole`, `ActiveOnAssignment` |
| Audit Comments | Card comments | `TrackerCommentId`, `TrackerAssignmentId`, `CommentCreatedAt`, `CommentAuthor`, `CommentBody` |
| Audit Checklist Items | Per-stage checklist state | `TrackerAssignmentId`, `ChecklistKey`, `ChecklistStage`, `ChecklistItem`, `Completed` |
...
7. Add Power Automate flows only after live list writes and activity-log entries are stable.
